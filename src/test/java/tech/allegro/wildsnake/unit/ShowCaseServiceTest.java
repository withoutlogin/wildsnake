...
public class ShowCaseServiceTest {
    ...
    public void getShowCaseItemsShouldReturnItems() throws Exception {
        ...
        List<Product> products = Arrays.asList(new Product("title1", "http://localhost/image1", "description1", 1240l), new Product("title2", "http://localhost/image2", "description2", 1560l));
        when(productRepository.findFirst3ByOrderByIdDesc()).thenReturn(products);

        List<ShowcaseItem> expectedItems = Arrays.asList(new ShowcaseItem("title1", "http://localhost/image1", 1240l), new ShowcaseItem("title2", "http://localhost/image2", 1560l));
